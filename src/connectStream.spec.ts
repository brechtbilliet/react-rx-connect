...
describe("connectStream", () => {
    test("should throw when no stream is specified", () => {
        ...
        expect(() => {connectStream(null, caller, key)}).toThrowError(`The passed stream for ${key} is not a valid RxJS stream`);
    });
    ...
    test("should throw when the first argument is not a valid observable", () => {
        ...
        expect(() => {connectStream(badStream$, component, key)}).toThrowError(`The passed stream for ${key} is not a valid RxJS stream`);
    });
    ...
    test("should throw an error when the specified key does not exist in the state object", () => {
        ...
        expect(() => {connectStream(stream$, component, key)}).toThrowError(`key ${key} does not exist on the state`);
    });
